...
<script type="application/json" id="google-maps-coords">
  [<?php
for ($x = 1; $x <= 187; $x++) {
echo    '{"location_name":"Location: '.$city_list[$x][0].'<br>Time of Sunraise: '.date('H:i:s', date_sunrise(time(),SUNFUNCS_RET_TIMESTAMP,$city_list[$x][1], $city_list[$x][2])).'","latitude":"'.$city_list[$x][1].'","longitude":"'.$city_list[$x][2].'"}';if($x<187)echo ','; 
}
